...
psql -U postgres -c "CREATE TABLE GPS (GROUPNAME TEXT NOT NULL, USERNAME TEXT NOT NULL REFERENCES CREDS(USERNAME)"
psql -U postgres -c "CREATE TABLE CHATROOMS(USERNAME TEXT NOT NULL REFERENCES CREDS(USERNAME), BUDDY TEXT NOT NULL REFERENCES CREDS(USERNAME)"
